...
HANAMI_ROM_DB = Sequel.connect("sqlite::memory:")

# Create the identities table
HANAMI_ROM_DB.create_table?(:hanami_identities) do
  primary_key :id
...
class HanamiIdentities < ROM::Relation[:sql]
  schema(:hanami_identities, infer: true) do
...
HANAMI_ROM_CONFIG = ROM::Configuration.new(:sql, HANAMI_ROM_DB)
HANAMI_ROM_CONFIG.register_relation(HanamiIdentities)
HANAMI_ROM_CONTAINER = ROM.container(HANAMI_ROM_CONFIG)

# HanamiIdentity class using ROM adapter
class HanamiIdentity
  include OmniAuth::Identity::Models::Rom

  # Configure ROM
  rom_container HANAMI_ROM_CONTAINER
  rom_relation_name :hanami_identities
  ...
  class << self
    ...
    def create(attributes)
      ...
      begin
        HANAMI_ROM_DB[:hanami_identities].insert(attributes)
    ...
    def delete_all
      HANAMI_ROM_DB[:hanami_identities].delete
    end
  end
end
